...
async function toProps (input: unknown, id: string): Promise<Props> {
  return await Apps.toPropsHelper(input, {
    ...
    stickyBlocksLazyLoadDistance: i => Apps.ifNotUndefinedHelper(i, Cast.toNumber),
    stickyBlocksViewportHeight: i => Apps.ifNotUndefinedHelper(i, Cast.toString),
    stickyBlocksOffsetTop: i => Apps.ifNotUndefinedHelper(i, Cast.toNumber),
    ...
    thresholdOffset: i => Apps.ifNotUndefinedHelper(i, Cast.toString),
    bgColorTransitionDuration: i => Apps.ifNotUndefinedHelper(i, Cast.toNumber),
...
async function arrayToPages (array: unknown[], id: string): Promise<PropsPageData[]> {
  ...
  for (const pageData of array) {
    ...
    const extractedPage: PropsPageData = await recordFormat(pageData, {
      id: i => Apps.ifNotUndefinedHelper(i, Cast.toString),
      bgColor: i => Apps.ifNotUndefinedHelper(i, Cast.toString),
...
async function arrayToBlocks (array: unknown[], id: string): Promise<PropsBlockData[]> {
  ...
  for (const blockData of array) {
    ...
    const strDepth = Apps.ifNotUndefinedHelper(blockData.depth, Cast.toString)
    ...
    if (strDepth === 'scroll' || strDepth === undefined) {
      const extractedScrollBlock: PropsScrollBlockData = await recordFormat(blockData, {
        depth: () => strDepth,
        id: i => Apps.ifNotUndefinedHelper(i, Cast.toString),
        zIndex: i => Apps.ifNotUndefinedHelper(i, Cast.toNumber),
        ...
        trackScroll: i => Apps.ifNotUndefinedHelper(i, Cast.toBoolean),
        layout: i => Apps.ifNotUndefinedHelper(i, Cast.toString) as LayoutName | undefined,
        mobileLayout: i => Apps.ifNotUndefinedHelper(i, Cast.toString) as LayoutName | undefined
      })
      ...
      const extractedStickyBlock: PropsStickyBlockData = await recordFormat(blockData, {
        ...
        transitions: i => Apps.ifArrayHelper(i, i => {
          ...
          i.forEach((transitionObj: unknown) => {
            ...
            const strName = Cast.toString(name)

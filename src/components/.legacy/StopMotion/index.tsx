...
class StopMotion extends Component<Props, {}> {
  ...
  componentDidUpdate(prevProps: Props): void {
    ...
    const pWidth = prevProps.width
    if (width !== pWidth) this.setCanvasSize()
    this.drawCurrentFrameOnCanvas()
  }

  getIndexBasedOnProgression(): number {
    if (this.props.images === undefined) return 0
  ...
  getFrameBasedOnProgression(): HTMLImageElement | undefined {
  ...
  async preloadImages() {
    ...
    while (imagesBefore.length > 0 || imagesAfter.length > 0) {
      if (imagesAfter.length) {
        imagesInOrder.push({ index: indexAfter, url: imagesAfter.shift() })
        indexAfter++
        continue
      }
      if (imagesBefore.length) {
        imagesInOrder.push({ index: indexBefore, url: imagesBefore.shift() })
        indexBefore--
        continue
      }
      break
    ...
    for (const { index, url } of imagesInOrder) {
      ...
      try {
        ...
        if (index === currentIndex) { this.imageRatio = img.height / img.width }
        // une fois qu'on chargé toutes les images
        if (this.imageElements.length === this.props.images.length) { this.setState({ loading: false }) }
      } catch (error) {
        console.log(error)
      }
  ...
  drawImageOnCanvas(image: HTMLImageElement): void {
    ...
    canvas.getContext('2d')?.drawImage(
      image, 0, 0, image.width, image.height, // ce qu'on dessine
      0, 0, canvas.width, canvas.height // où on le dessine
    )
  }
  ...
  render(): (JSX.Element | null) {
    ...
    return <ResizeObserver onResize={this.setCanvasSize}>
      {/* [WIP] Max : je pense que resize observer ne veut qu'un seul élément enfant */}

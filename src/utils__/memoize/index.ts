...
export default function memoize<T extends BasicFunc = BasicFunc> (toMemoizeFunc: T) {
  ...
  return function memoized (...args: Parameters<T>) {
    const allArgsAreInCache = cachedArgs !== undefined && args.every((arg, argPos) => (cachedArgs as Parameters<T>)[argPos] === arg)
    const allCachedAreInArgs = cachedArgs !== undefined && cachedArgs.every((arg, argPos) => args[argPos] === arg)

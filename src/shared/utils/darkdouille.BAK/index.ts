...
export namespace Darkdouille {
  ...
  export function isTransformerElement (node: Node): node is Element {
    ...
    if (!isElement) return false
    const typeTag = node.tagName.toLowerCase()
    const typeAttr = node.getAttribute('type') // [WIP] remove the possibility of <data type="number"
  ...
  export function getTypeFromElement (element: Element): Exclude<Type, Type.DATA> | null {
    ...
    const typeAttr = element.getAttribute('type') // [WIP] remove the possibility of <data type="number"
  ...
  export function reduce (element: Element): Element {
    ...
    childNodes.forEach(childNode => {
      ...
      valueElement.remove()
      if (type !== null) existingElement.setAttribute('type', type) // [WIP] remove the possibility of <data type="number"
  ...
  export class Tree {
    ...
    get children (): NodeListOf<Node> | { [key: string]: Tree } | Tree[] {
      ...
      if (namedChildItems.length > 0) {
        ...
        children = htmlChildItems.map((childItem, pos) => {
          if (childItem.type === 'text') {
            const wrapper = document.createElement('string')
            wrapper.innerHTML = childItem.node.textContent ?? ''
            return new Tree(wrapper, this, `${(this.path ?? '')}/${pos}`)
          } else return new Tree(childItem.element, this, `${(this.path ?? '')}/${pos}`)
        })
    ...
    get value (): TreeValue {
      ...
      if (className !== null) console.group(`<${tag} class="${className}" />`)
      else console.group(`<${tag} />`)
      const { untransformedValue, masterTransformer } = this
      console.log(untransformedValue)

...
export const at = SmartTags.makeSmartTag<Main, Args, Output>({
  ...
  argsValueCheck: a => {
    ...
    if (a.length === 0) return Outcome.makeFailure(makeArgsValueError('number', 'undefined', 0))
    if (a.length > 1) return Outcome.makeFailure(makeArgsValueError('undefined', getType(a[1]) ?? 'undefined', 1))
    const checked = typeCheckMany(a, 'number')
  ...
  func: (main, args) => {
    ...
    if (typeof main === 'string'
      || Array.isArray(main)
      || main instanceof NodeList) { found = main[pos] }
    else {

...
export const getproperty = SmartTags.makeSmartTag<Main, Args, Output>({
  ...
  argsValueCheck: a => {
    ...
    if (a.length !== 1) return Outcome.makeFailure({ position: 1, expected: 'undefined', found: Utils.Tree.TypeChecks.getType(a.at(1)) ?? 'undefined' })
  ...
  func: (main, args, { name, sourceTree }) => {
    ...
    const valType = Utils.Tree.TypeChecks.getType(val)

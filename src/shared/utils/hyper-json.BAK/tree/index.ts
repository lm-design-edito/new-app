...
export namespace Tree {
  ...
  export class Tree<T extends Element | Text = Element | Text> {
    ...
    constructor (node: T, parent: Tree | null, pathFromParent: string | number | null, options: Partial<Types.Tree.Options> = defaultOptions) {
      ...
      const hasModeAttribute = this.attributes?.find(({ name, value }) => {
        if (name !== modeAttribute) return false
        if (value.trim().toLocaleLowerCase() === 'coalescion') return true
        if (value.trim().toLocaleLowerCase() === 'isolation') return true
        return false
      })
      if (hasModeAttribute !== undefined) { this.mode = hasModeAttribute.value as 'coalescion' | 'isolation' }
      else if (this.smartTagData.defaultMode !== undefined) { this.mode = this.smartTagData.defaultMode }
      else { this.mode = 'isolation' }
      const modeAttributeValue = hasModeAttribute
        ? hasModeAttribute.value as 'coalescion' | 'isolation'
        : 'isolation'
      this.mode = modeAttributeValue

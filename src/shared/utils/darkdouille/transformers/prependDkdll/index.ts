...
const prependDkdll: Darkdouille.TransformerFunctionGenerator<Darkdouille.TreeValue> = (...args) => {
  return (inputValue): Darkdouille.TreeValue => {
    ...
    targets.forEach(target => {
      dkdllValues.forEach(dkdllVal => {
        if (typeof dkdllVal === 'string') target.prepend(dkdllVal)
        else target.prepend(...dkdllVal)
      })
    })

...
async function main () {
  ...
  * * * * * * * * ** * * * * * * * * * * * */
  ...
  const dstProdSharedAppendedContent = `
    ...
    window.LM_PAGE.meta.buildTime = '${new Date().toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      weekday: 'short',
    ...
    window.LM_PAGE.meta.rootUrl = '${targetToRootUrlMap.get(targetDestinationName)}';
    window.LM_PAGE.meta.target = '${targetDestinationName.split('/').at(-1)}';
    window.LM_PAGE.meta.version = '${targetVersionStr}';
